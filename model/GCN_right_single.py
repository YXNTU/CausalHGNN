...
class GCN_right_single(BaseModel):
    def __init__(self, in_dim,
                 ...
                 batchnorm=False):
        super(GCN_right_single, self).__init__()
        self.in_dim = in_dim
        ...
        self.fc_list = nn.ModuleDict({
            str(i): nn.Sequential(
                nn.Linear(in_dim[i], hidden_dim, bias=True),
                nn.Sigmoid() if self.use_self_gating else nn.Identity()
            )
    ...
    def forward(self, hg, processed_features):
        ...
        for i, feature in enumerate(processed_features):
            ...
            if key in self.fc_list:
                embedded_feature = self.fc_list[key](feature)
                embedded_features.append(embedded_feature)
            else:
                raise KeyError(f"Feature list index '{key}' does not have a corresponding FC layer in fc_list.")

        # 将所有嵌入后的特征拼接到一起，形成单一的张量
        concatenated_features = th.cat(embedded_features, dim=0)  # 拼接操作，按最后一个维度拼接
        h_dict = {'0': concatenated_features}
